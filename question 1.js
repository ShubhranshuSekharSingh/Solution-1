function findCaliforniaCafes(searchTerm) {
 ...
 for (let i = 0; i < cafes.length; i++) {
  if (cafes[i].name.includes(searchTerm)) {
   let place_id = cafes[i].place_id;
   let name = cafes[i].name;
   for (let j = 0; j < places.length; j++) {
    if (places[j].id === place_id) {
     let street_no = places[j].street_no;
     let locality = places[j].locality;
     let postal_code = places[j].postal_code;
     let lat = places[j].lat;
     let long = places[j].long;
